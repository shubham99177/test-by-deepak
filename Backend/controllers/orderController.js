...
module.exports.placeOrder = async (req, res) => {
  ...
  try {
    // Find the user by userId and populate cart
    ...
    const order = {
      ...
      items: [
        {
          productId: cartItem.productId, // Reference to the product
          quantity: cartItem.quantity, // Quantity from the cart
          finalPrice, // Include finalPrice in the order details
        },
      ],
      orderDate: new Date(),
    };
...
module.exports.deleteOrder = async (req, res) => {
  ...
  try {
    ...
    // Find the index of the order in the orders array based on orderId (not _id)
    ...
    // Optionally, you can return the updated orders
    return res
      .status(200)
      .json({ message: "Order deleted successfully.", orders: user.orders });
  } catch (error) {
